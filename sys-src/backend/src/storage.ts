...
export default class Storage {
    ...
    public setup(): Storage {
        ...
        this.db.exec(
            'create unique index if not exists score_level_user_uindex\n' + '    on `score` (level, user);\n' + '\n',
        );
        return this;
    }

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    public get(nickname: string, handler: (rows: any[]) => void): Storage {
        const stmt = this.db.prepare(
            'select * from `user` inner join `score` on `user`.`id` == `score`.`user` where `user`.`nickname` == ?',
        );
        handler(stmt.all(nickname));
        return this;
    }

    public post(nickname: string, scores: { key: string; value: number }[]): Storage {
